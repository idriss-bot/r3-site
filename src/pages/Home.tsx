...
function BadgeWhatsApp({ size = 13, className }: { size?: number; className?: string }) {
  const isLarge = size >= 20;
  return (
    <span
      className={`inline-flex items-center align-middle rounded-full ${className || ''}`}
      style={{
...
function BadgeWhatsAppResponsive() {
  return (
    <span
      className="inline-flex items-center align-middle rounded-full px-2 py-0.5 sm:px-3 sm:py-1 md:px-3 md:py-1 text-lg sm:text-xl md:text-2xl lg:text-3xl gap-1 sm:gap-1.5 md:gap-2"
      style={{
        background: 'rgba(37,211,102,0.12)',
        color: '#25d366',
        fontWeight: 500,
    ...
    >
      <svg className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 lg:w-8 lg:h-8 shrink-0" viewBox="0 0 24 24" fill="#25d366">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/>
      </svg>
      WhatsApp
    </span>
    ...
    <div className="min-h-screen bg-bg text-white font-body selection:bg-primary selection:text-white relative overflow-x-hidden">
          ...
          <div className="flex flex-col lg:flex-row gap-20 items-center max-w-6xl mx-auto">
            ...
            <div className="w-full lg:w-3/5">
              <FadeIn delay={0.2}>
                ...
                <div className="space-y-6 text-base sm:text-lg md:text-xl text-gray-300 font-light leading-relaxed md:text-justify">
                  <p>Chasseur d'appartements à Paris de 2017 à 2022, je connais ce <span className="gold-gradient-text font-semibold">métier de l'intérieur</span> : la pression des mandats, les rendez-vous qui ne convertissent pas, <span className="whitespace-nowrap">le temps qui manque toujours.</span></p>
                  <p>Pendant le confinement, je me lance sur LinkedIn. En 3 mois, ma communauté passe de 240 à 7 000 abonnés. Je comprends une chose : <span className="gold-gradient-text font-semibold whitespace-nowrap">dans l'immobilier, celui qui est visible gagne.</span></p>
                  <p>En 2 ans, j'accompagne plus de 30 agents partout en France. Résultat : <span className="gold-gradient-text font-semibold whitespace-nowrap">plus d'un million d'euros de CA générés</span> via LinkedIn et Facebook, simplement avec du <span className="whitespace-nowrap">texte et des images.</span></p>
                  <p>En 2025, je fais pivoter mon activité vers la vidéo. Les temps de tournage étaient incompatibles avec le métier d'agent immobilier — <span className="gold-gradient-text font-semibold">l'IA a changé la donne</span>. J'ai conçu <span className="gold-gradient-text font-semibold">Mireille</span> pour permettre à mes clients d'être présents sur les réseaux <span className="gold-gradient-text font-semibold whitespace-nowrap">sans tournage, sans logistique, sans stress</span>.</p>
                </div>
              </FadeIn>
            </div>
          </div>
      ...
      <section id="offre" className="py-16 lg:py-24 bg-surface-light border-y border-white/5 relative">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <FadeIn delay={0.1}>
            <div className="max-w-xl mx-auto glass-panel border border-primary/30 rounded-3xl p-6 md:p-8 text-center relative shadow-[0_0_50px_rgba(194,129,53,0.15)] overflow-visible">
